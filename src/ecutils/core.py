...
class JacobianEllipticCurveOperations:
    """Implements mathematical operations for elliptic curves in Jacobian coordinates."""
    ...
    def add_points(self, p1: JacobianPoint, p2: JacobianPoint) -> JacobianPoint:
    ...
    def double_point(self, p: JacobianPoint) -> JacobianPoint:
    ...
    def multiply_point(self, k: int, p: JacobianPoint) -> JacobianPoint:
    ...
    @staticmethod
    def convert_to_jacobian(point: Point) -> JacobianPoint:
    ...
    def convert_to_affine(self, point: JacobianPoint) -> Point:
...
@dataclass
class JacobianEllipticCurve(JacobianEllipticCurveOperations):
    """Represents the parameters and operations of an elliptic curve in Jacobian coordinates.
...
class EllipticCurveOperations:
    ...
    def add_points(self, p1: Point, p2: Point) -> Point:
        ...
        if self.use_jacobian:
            p1_jacobian = self.jacobian.convert_to_jacobian(p1)
            p2_jacobian = self.jacobian.convert_to_jacobian(p2)
            p3_jacobian = self.jacobian.add_points(p1_jacobian, p2_jacobian)
            return self.jacobian.convert_to_affine(p3_jacobian)
    ...
    def multiply_point(self, k: int, p: Point) -> Point:
        ...
        if self.use_jacobian:
            p_jacobian = self.jacobian.convert_to_jacobian(p)
            q_jacobian = self.jacobian.multiply_point(k, p_jacobian)
            p1 = self.jacobian.convert_to_affine(q_jacobian)
            if p1.x is None or p1.y is None:
                return p1
...
class EllipticCurve(EllipticCurveOperations):
    ...
    Attributes:
        ...
        h (int): The cofactor.
        use_jacobian (bool): If True, Jacobian coordinates will be used in curve operations.
        jacobian (JacobianEllipticCurve): Optional. If set, Jacobian coordinates will be used in curve operations.
    """
    ...
    jacobian: JacobianEllipticCurve = None

    def __post_init__(self) -> None:
        if self.use_jacobian:
            self.jacobian = JacobianEllipticCurve(p=self.p, a=self.a, b=self.b)
